...
# --- Constantes ---
ESP32_IP_ADDRESS = "0.0.0.0" # IP do Access Point do DroneBridge
MAVLINK_PORT = 14550
# A porta de comando não é mais necessária
# COMMAND_PORT = 8000 
CONNECTION_STRING = f"udpin:{ESP32_IP_ADDRESS}:{MAVLINK_PORT}"
...
LOW_VOLTAGE_THRESHOLD = 6.2  # Tensão em Volts para pouso de emergência


ESP_CAM_IP = "192.168.2.2" 
CAMERA_STREAM_URL = f"http://{ESP_CAM_IP}"
...
class ApiFacade:
    ...
    def drone_reconnect(self): return self.api.drone_reconnect()
    ...
    def drone_set_gpio(self, pin, state): return self.api.drone_set_gpio(pin, state)
    def open_camera_window(self): return self.api.open_camera_window()
...
class JsApi:
    def __init__(self):
        ...
        self.telemetry_paused = threading.Event()
    ...
    def open_camera_window(self):
        """
        Abre uma SEGUNDA janela do pywebview para exibir o stream da câmera.
        """
        if not CAMERA_STREAM_URL:
            print("URL da câmera não configurada.")
            if self.window:
                self.window.evaluate_js('alert("URL da câmera não configurada no main.py")')
            return

        print(f"Abrindo janela da câmera em {CAMERA_STREAM_URL}")
        try:
            # O 'gui=self.window.gui' é importante para usar o mesmo backend (EdgeChromium)
            webview.create_window(
                'Stream da Câmera ESP-CAM',
                CAMERA_STREAM_URL,
                width=640,
                height=480, 
            )
        except Exception as e:
            print(f"Erro ao abrir janela da câmera: {e}")
            if self.window:
                self.window.evaluate_js(f'alert("Erro ao abrir janela da câmera: {e}")')
                
                
    def drone_set_gpio(self, pin, state):
        if drone:
            print(f"Encaminhando comando GPIO via MAVLink para o pino {pin}...")
            drone.set_gpio(pin, state)
    ...
    def _telemetry_loop(self):
        while not self.stop_event.is_set():
            ...
            if self.telemetry_paused.is_set():
                time.sleep(0.5)               
                continue
            ...
            # Loop interno para processar todas as mensagens MAVLink pendentes de uma vez
            while True:
                msg = drone.master.recv_match(type=['ATTITUDE', 'VFR_HUD', 'SYS_STATUS', 'HEARTBEAT', 'GPS_RAW_INT', 'DISTANCE_SENSOR', 'BATTERY_STATUS', 'STATUSTEXT'], blocking=False)
                
                if not msg: 
                    break # Sai do loop interno se não houver mais mensagens

                msg_type = msg.get_type()
                
                if msg_type == 'STATUSTEXT':
                    message_text = json.dumps(msg.text)
                    if self.window:
                        try:
                            self.window.evaluate_js(f'addLogMessage({message_text})')
                        except Exception as e:
                            print(f"Erro ao enviar log para UI: {e}")
                
                # Atualiza o dicionário de telemetria com base em cada mensagem
                with self.telemetry_lock:
                    if msg_type == 'ATTITUDE':
                        ...
                        self.telemetry_data['altitude'] = round(msg.current_distance / 100.0, 2)
                    elif msg_type == 'VFR_HUD':
                        if self.telemetry_data.get('altitude', 0) == 0: 
                        ...
                        if msg.voltages[0] < 65535: 
                            self.telemetry_data['battery_volt'] = msg.voltages[0] / 1000.0
                    elif msg_type == 'SYS_STATUS':
                        self.telemetry_data['battery_perc'] = msg.battery_remaining
                        if self.telemetry_data.get('battery_volt', 0) == 0: 
            ...
            # --- CORREÇÃO PRINCIPAL ---
            # A atualização do HUD agora acontece AQUI, fora do loop 'while True',
            # garantindo que todos os dados foram coletados antes de desenhar.
            if self.window:
                try: 
                    self.window.evaluate_js(f'update_hud({json.dumps(self.telemetry_data)})')
                except Exception: 
                    pass
            
            # O bloco do Failsafe vem em seguida
            with self.telemetry_lock:
                current_voltage = self.telemetry_data.get('battery_volt', 0)
                is_armed = self.telemetry_data.get('armed', False)

                if is_armed and current_voltage > 0 and current_voltage < LOW_VOLTAGE_THRESHOLD and not self.battery_failsafe_triggered:
                    self.battery_failsafe_triggered = True
                    print(f"!!! BATERIA CRÍTICA ({current_voltage:.2f}V) !!! ACIONANDO POUSO DE EMERGÊNCIA.")
                    if self.window:
                        self.window.evaluate_js('alert("ALERTA: Bateria crítica! O drone irá pousar agora.")')
                    self.drone_abort_mission()
                    self.drone_land()

                if not is_armed and self.battery_failsafe_triggered:
                    print("Drone desarmado. Failsafe de bateria redefinido.")
                    self.battery_failsafe_triggered = False

            # Pausa única no final do ciclo principal
            time.sleep(0.1)
    ...
    def drone_reboot_autopilot(self):
        """Inicia o processo de reinicialização em uma nova thread para não travar a UI."""
        if not drone:
            print("Drone não conectado. Não é possível reiniciar.")
            return
        
        # O confirm() do JavaScript já garante que o usuário quer reiniciar
        threading.Thread(target=self._reboot_and_reconnect_sequence, daemon=True).start()

    def _reboot_and_reconnect_sequence(self):
        """Gerencia o processo de enviar o comando, pausar a telemetria, esperar e reconectar."""
        # 1. Pausa a thread de telemetria para evitar conflitos
        self.telemetry_paused.set()
        print("Thread de telemetria pausada.")

        try:
            # Limpa os dados antigos para evitar confusão na UI
            with self.telemetry_lock:
                self.telemetry_data['mode'] = 'REINICIANDO'
                self.telemetry_data['armed'] = False
            
            # Envia a atualização final antes de reiniciar
            if self.window:
                self.window.evaluate_js(f'update_hud({json.dumps(self.telemetry_data)})')
            ...
            if self.window:
                message = json.dumps("Comando enviado. Aguardando 15s para o drone reiniciar e reconectar...")
                self.window.evaluate_js(f'addLogMessage({message})')

            time.sleep(15)

            if self.window:
                message = json.dumps("Tentando reconectar...")
                self.window.evaluate_js(f'addLogMessage({message})')

            if drone.reconnect():
                if self.window:
                    message = json.dumps("Reconexão bem-sucedida!")
                    self.window.evaluate_js(f'addLogMessage({message})')
            else:
                 if self.window:
                    message = json.dumps("ERRO: Falha ao reconectar. Verifique o drone.")
                    self.window.evaluate_js(f'addLogMessage({message})')

        except Exception as e:
            print(f"Ocorreu um erro durante a sequência de reinicialização: {e}")
            if self.window:
                message = json.dumps(f"ERRO na reinicialização: {e}")
                self.window.evaluate_js(f'addLogMessage({message})')
        finally:
            # 2. Libera a thread de telemetria, não importa se deu certo ou errado
            self.telemetry_paused.clear()
            print("Thread de telemetria retomada.")
